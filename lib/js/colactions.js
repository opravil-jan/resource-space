...
function loadCompactSelector(ref,tag,pagename,col_min){
	if (col_min){col_min='true';}
	jQuery.ajax({
		type: 'GET',
		url:  baseurl_short+'pages/collections_compact_style.php?collection='+ref+'&pagename='+pagename+'&colselectload=true+col_min='+col_min,
...
function colAction(value){
	...
	if (confirmed){
		if (value[2]!="0"){
			var wait= jQuery.ajax(value[2],{async:true});
		}
		if (value[5]=='main'){ 
