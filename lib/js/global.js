...
function ModalCentre()
    ...
    if (modalalign=='right')
	{
        left = Math.max(jQuery(window).width() - jQuery('#modal').outerWidth(), 0) - 130;
	topmargin=50;
	}
    else
	{
...
function ActivateHeaderLink(activeurl)
		{
		currentpage=activeurl.split(/[?#]/)[0];
		jQuery('#HeaderNav2 li a').each(function(){
			linktarget=this.href.split(/[?#]/)[0];
			if(currentpage.indexOf(linktarget)>-1){
