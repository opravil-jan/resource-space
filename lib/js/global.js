...
function CentralSpaceLoad (anchor,scrolltop)
	...
	//jQuery( document ).off('rs_pageready');
	CentralSpace.load(url, function (response, status, xhr)
		...
		if (status=="error")
			...
			if(typeof(top.history.pushState)=='function')
				...
				var state = {
						title: newtitle,
						content: CentralSpace.html()
					}
				top.history.pushState(state, newtitle, anchor.href);
				console.log(state);
		...
		//jQuery(document).trigger('rs_pageready', { page:pagename });
...
top.window.onpopstate = function(event)
	...
	// clear rs_pageready event listeners
	//jQuery( document ).off('rs_pageready', { page:pagename });

	var page       = event.state.content;
...
function CentralSpacePost (form,scrolltop)
	...
	jQuery.post(url,jQuery(form).serialize(),function(data)
		...
		if(typeof(top.history.pushState)=='function')
			...
			top.history.pushState(state, applicationname, form.action);
...
function CollectionDivLoad (anchor,scrolltop)
	...
	// remove rs_pageready handlers
	//jQuery( document ).off('rs_pageready');
	
	jQuery('#CollectionDiv').load(url, function ()
		{
		
		//jQuery('#CollectionDiv').trigger('rs_pageready', { page:pagename });
