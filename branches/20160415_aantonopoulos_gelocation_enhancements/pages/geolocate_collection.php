<?php
include "../include/db.php";
include_once "../include/general.php";
include "../include/authenticate.php"; 
include "../include/resource_functions.php";
include "../include/collections_functions.php";
include "../include/header.php";

$ref=getvalescaped("ref","",true);
$all_resources =  get_collection_resources($ref) ;
//print_r($all_resources );
echo "<br>";
$markers = array();
$mean_lat=0;
$mean_long=0;
foreach ($all_resources as $value) {
    #echo $value;
    $resource = get_resource_data($value,$cache=true);
    //echo $resource['geo_long'] . " " . $resource['geo_lat'] ;
    $markers[] =  [ $resource['geo_long'] . "," .  $resource['geo_lat'] ];
    $mean_lat=$mean_lat+$resource['geo_lat'];
    $mean_long=$mean_long+$resource['geo_long'];

}
$mean_lat=$mean_lat/count($markers);
$mean_long=$mean_long/count($markers);
print_r($markers);

 echo json_encode($markers, JSON_NUMERIC_CHECK) ;

?>

<h1><?php echo $lang["geolocatecollection"] ?></h1>

  <div id="GeoColDiv" style="width:800px; height:300px;></div>
 <script src="http://www.openlayers.org/api/OpenLayers.js"></script>
  <script>
    map = new OpenLayers.Map("GeoColDiv");
    map.addLayer(new OpenLayers.Layer.OSM());
    epsg4326 =  new OpenLayers.Projection("EPSG:4326"); //WGS 1984 projection
    projectTo = map.getProjectionObject(); //The map projection (Spherical Mercator)
   
    var lonLat = new OpenLayers.LonLat(  <?php echo $mean_long . "," . $mean_lat;?> ).transform(epsg4326, projectTo);
          
    var zoom=14;
    map.setCenter (lonLat, zoom);

    var vectorLayer = new OpenLayers.Layer.Vector("Overlay");
    
    // Define an array. This could be done in a seperate js file.
    // This tidy formatted section could even be generated by a server-side script (jsonp)

    var markers = <?php echo json_encode($markers, JSON_NUMERIC_CHECK) ?>;/*[[-5.9909138888889,37.382869444444],[-5.9916222222222,37.393844444444]];*/


    //Loop through the markers array
    for (var i=0; i<markers.length; i++) {
      
       var lon = parseFloat(markers[i][0]);
       var lat = parseFloat(markers[i][1]);
       alert(lon,lat);
        var feature = new OpenLayers.Feature.Vector(
                new OpenLayers.Geometry.Point( lon, lat ).transform(epsg4326, projectTo),
                {description: "marker number " + i} ,
                {externalGraphic: 'http://localhost/resourcespace/lib/OpenLayers/img/marker.png', graphicHeight: 25, graphicWidth: 21, graphicXOffset:-12, graphicYOffset:-25  }
            );             
        vectorLayer.addFeatures(feature);
    }                        
    
    map.addLayer(vectorLayer);
</script>


<?php
include "../include/footer.php";
?>
